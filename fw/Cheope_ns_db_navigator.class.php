...
//
// Questa classe è impostata per funzionare solo come albero. 
//

class Cheope_ns_db_navigator extends Cheope_ns_tree_ctrl
{
 
 const ERROR_1="Cheope_ns_db_navigator.putMenuPars:Cardinalità degli id errata.";
 const ERROR_2="Cheope_ns_db_navigator.putMenuPars:Numero campi dati errato.";
 const ERROR_3="Cheope_ns_db_navigator.putMenuPars:Struttura del database.";
 const ERROR_4="Cheope_ns_db_navigator.putData:Oggetto dati errato.";
// const ERROR_5="Cheope_ns_db_navigator.putData:Numero campi dati errato."; 
...
function putMenuPars(array $actDataValues):void
{
 ...
 $htmlWriter->putGenericHtmlString("var " . $intCode . VAR_SEP . "nVet = 1;"); 
 $htmlWriter->putGenericHtmlString(STRING_NULL); 

 $htmlWriter->putGenericHtmlString("var " . $intCode . VAR_SEP . 
 self::JAVASCRIPT_VAR_LABEL . VAR_SEP . "0" . " = " . "\"" . $rootLabel . "\"" . STRING_SEMICOLON);		
 $htmlWriter->putGenericHtmlString("var " . $intCode . VAR_SEP . 
 self::JAVASCRIPT_VAR_PAR . VAR_SEP . "0" . " = " . "\"" . STRING_CANCELLETTO .  "\"" . STRING_SEMICOLON); 	
 
 $num1 = count($labels);

 $htmlWriter->putGenericHtmlString("var " . $intCode . VAR_SEP . 
 self::JAVASCRIPT_VAR_LABELS . VAR_SEP . "0" . " = new Array(" . $num1 . ")" . STRING_SEMICOLON); 
 $htmlWriter->putGenericHtmlString("var " . $intCode . VAR_SEP . 
 self::JAVASCRIPT_VAR_URLS . VAR_SEP . "0" . " = new Array(" . $num1 . ")" . STRING_SEMICOLON);
 ...
 for($j=0;$j<=$num1-1;$j++)
 {
  ...
  $num=0;
	   ...
	   $linkTableObj = $dbStruct->getElementByAliasName($linkTable);
				...
				//  echo "WWWW";
				 ...
				 // echo "WWWW";
				  ...
				  //echo "WWWW";
				 // echo "XXXXXX";
		 $row1 = $linkTableObj->getFilteredOrdData($keyField,
		 ...
		 {			
			...
			for($k=0;$k<=$num2-1;$k++)
			{
				...
				echo getType($row1[$k]);
				echo "WWWWW";*/
			 $row2 = $nextObj->getFilteredOrdData($keyField,(int)$row1[$k],
		   $ordField,$field);
	...
	$htmlWriter->putGenericHtmlString("var " . $intCode . VAR_SEP . 
	self::JAVASCRIPT_VAR_LABELS . VAR_SEP . "0" . VAR_SEP . $j . " = new Array(" . $num . ")" . STRING_SEMICOLON); 
	$htmlWriter->putGenericHtmlString("var " . $intCode . VAR_SEP . 
	self::JAVASCRIPT_VAR_URLS . VAR_SEP . "0" . VAR_SEP . $j . " = new Array(" . $num . ")" . STRING_SEMICOLON);
