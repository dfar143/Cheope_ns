...
class Cheope_ns_vert_local_tabs extends Html_data_interface
{ 
 const ERROR_3="Vert_simple_local_tabs.putData:Numero campi dati errato.";
 const VOICES_POS=0;
 const DEFAULT_CSS_CLASS="vert_tab";
 const DEFAULT_CSS_MODULE="vert_local_tabs";
  ...
  function __construct($actObj=OBJ_NONE,string $actOp=OP_NONE,$actNum=STRING_NULL)
 {
  ...
  parent::__construct($actObj,$actOp,self::INT_VERT_LOCAL_TABS,$actNum);
  $this->setCssModule(CLIENT_STYLE_SHEET_PATH . DIR_SEP . 
  self::DEFAULT_CSS_MODULE . STYLE_SHEET_FILE_POSTFIX); 
  $this->setJavascriptModule(CLIENT_VERT_LOCAL_TABS_CODE_PATH . DIR_SEP . JS_VERT_LOCAL_TABS);
 }
 ...
 function putContainer(array $actDataValues):void
 {
  ...
  foreach($labels as $ind=>$label)     
  {
	...
	STRING_NULL,STRING_NULL,STRING_NULL,"vert_local_tabs_openCity(event, '" . $label . "')"); 
  ...
  foreach($labels as $ind=>$label)
  {
    ...
    {		
	 $htmlWriter->putDivOpenTag($labels[$i],STRING_NULL,"vert_tabcontent");
	 $ctl->putData();
	 $htmlWriter->putGenericHtmlString(DIV_CLOSE_TAG);
	}
    $iterator->next();
