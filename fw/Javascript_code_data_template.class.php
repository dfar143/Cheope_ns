...
class Javascript_code_data_template extends Html_data_interface
{ 
// const BODY_ID_SUFFIX="body";
 const DEFAULT_INHERIT_COUNT_FIELD_NAME="COUNT";
// const DEFAULT_CSS_CLASS="data_template";
 const ERROR_1="Javascript_code_data_template:Il nome di campo COUNT è riservato.";
...
// private $divEnvelope = false;
 ...
 function __construct($actObj=OBJ_NONE,string $actOp=OP_NONE,$actNum=STRING_NULL)
 {
  ...
  if($actNum === STRING_NULL)
 	...
 	parent::__construct($actObj,$actOp,Interfaces_info::INT_JAVASCRIPT_CODE_DATA_TEMPLATE,$actNum);
 }
 ...
 function serialize():void
 {
	parent::serialize();
 	...
 	$item1 = array("@javascriptTemplate"=>$javascriptTemplate);
 	$serializer->loadItems($item1);	
 }
...
/* function setDivEnvelope(string $actDivEnvelope)
 ...
 function loadTemplateFromFile(string $actFileName):void
 {
 	...
 	$javascriptTemplate = STRING_NULL;
 	foreach($fileRows as $fileRow)
 	{
 	 $javascriptTemplate = $javascriptTemplate . $fileRow;
 	}
 	$this->setJavascriptTemplate($javascriptTemplate);
 }
 ...
 function action(string $actStr,Interfaces_container $actInterfacesContainer):void
 {
  ...
  $this->setJavascriptTemplate($actStr);
 }
...
//
    ...
    $fieldValue->putData();
 	  $javascriptTemplate = preg_replace("/\{" . strToUpper($field) . "\}/",
 	  ((is_a($fieldValue,Classes_info::HTML_FORMATTED_INTERFACE_CLASS))?
 	  ($fieldValue->getHtmlWriter()->getItemStack()->flush()):
 	  ($fieldValue->getItemStack()->flush())),$javascriptTemplate);  
    $javascriptTemplate = preg_replace("/\{" . 
    self::DEFAULT_INHERIT_COUNT_FIELD_NAME . "\}/",$actNum,$javascriptTemplate);
//    
...
// 
   ...
   {
 	  $javascriptTemplate = preg_replace("/\{" . strToUpper($field) . "\}/",$fieldValue($actNum,$field),$javascriptTemplate); 	    
	 }
 	 else
 	 {
 	  $javascriptTemplate = preg_replace("/\{" . strToUpper($field) . "\}/",$fieldValue,$javascriptTemplate);
    $javascriptTemplate = preg_replace("/\{" . 
    self::DEFAULT_INHERIT_COUNT_FIELD_NAME . "\}/",$actNum,$javascriptTemplate);
   }
  }
...
// $style = $this->getStyle();	
  ...
  $javascriptTemplate = $this->getJavascriptTemplate();
  $javascriptTemplateInstance = STRING_NULL;
// 	$cssClass = $this->getCssClass();
  ...
  if(count($rows)>0)
  {
   ...
   {
   	$javascriptTemplate = $this->elabFields($rowVal,$i);
   	if($i==0)
   	 $javascriptTemplateInstance = $javascriptTemplate;
   	else
     $javascriptTemplateInstance = $javascriptTemplateInstance . STRING_ESC_RETURN . $javascriptTemplate;
    $i++;   
   }
  }
  else
  {
   $javascriptTemplateInstance = $this->elabFields($rows,$i);
  }
//  $divEnvelope = $this->getDivEnvelope();
//  if($divEnvelope)
//   $htmlWriter->putDivOpenTag($intCode,$style,$cssClass);
 	$htmlWriter->putGenericHtmlString($javascriptTemplateInstance);
// 	if($divEnvelope)
