...
function getMaxId(string $actTabName,string $actKeyField):int
{
  ...
  if(!$conn)
  {
   ...
   die(SQLSRV_CONNECTION_ERROR);
	...
	die(SQL_SERVER_FUNCTION_GETMAXID_ERROR_1  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
...
function deleteDbRow(string $actTab,string $actKey,int|string $actId):void
{
 ...
 $conn = sqlsrv_connect(SQLSRV_HOST,$sqlsrv_connectionInfo);
  ...
  if(!$conn)
  {
   die(SQLSRV_CONNECTION_ERROR  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
  }


 $result = sqlsrv_query($conn,$query) or die(SQL_SERVER_FUNCTION_DELETEDBROW_ERROR_1  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
 sqlsrv_free_stmt($result);
 sqlsrv_close($conn);	
 //return true;
}

function deleteDbRows(string $actTab,string $actField,int|string $actVal):int|false
{
 ...
 $result = sqlsrv_query($conn,$query) or die(SQL_SERVER_FUNCTION_DELETEDBROWS_ERROR_1  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
...
function updateDbRow(string $actTab,string $actKeyField,array $actRow):void{
  ...
  if($i==0)
	{
	 if(is_null($fieldValue))
		 $cols = $fieldName . STRING_EQUAL . 'NULL';
	 elseif (is_string($fieldValue))
	   $cols = $fieldName . STRING_EQUAL . 
	   STRING_SINGLE_QUOTE . $fieldValue . STRING_SINGLE_QUOTE;
	 else
	   $cols = $fieldName . STRING_EQUAL . $fieldValue;
	 $i++;
	} 
	else
	{
	 if(is_null($fieldValue))
		 $cols = $cols . STRING_COMMA . $fieldName . STRING_EQUAL . 'NULL';
	 elseif (is_string($fieldValue))
     $cols = $cols . STRING_COMMA . $fieldName . STRING_EQUAL . 
     STRING_SINGLE_QUOTE . $fieldValue . STRING_SINGLE_QUOTE;
	 else
	   $cols = $cols . STRING_COMMA . $fieldName . STRING_EQUAL  . $fieldValue;
	}
 }	
 ...
 $result = sqlsrv_query($conn,$query) or die(SQL_SERVER_FUNCTION_UPDATEDBROW_ERROR_1  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
...
function insertDbRowGetIdentity(string $actTab,array $actRow,string &$actId):void
{
  ...
  if($i==0)
	{
	 ...
	 if(is_null($fieldValue))
	   $vals = 'NULL';
	 elseif(is_string($fieldValue))
	   $vals = STRING_SINGLE_QUOTE . $fieldValue . STRING_SINGLE_QUOTE;
	 else
	   $vals = $fieldValue;
	 $i++;	 
	} 
	...
	{
   $cols = $cols . STRING_COMMA . $fieldName;
	 if(is_null($fieldValue))
	   $vals = $vals . STRING_COMMA . 'NULL';
   elseif(is_string($fieldValue))
     $vals = $vals . STRING_COMMA . STRING_SINGLE_QUOTE . $fieldValue . STRING_SINGLE_QUOTE;
	 else
	   $vals = $vals . STRING_COMMA . $fieldValue;
	}
 }	
 ...
 $result = sqlsrv_query($conn,$query) or die(SQL_SERVER_FUNCTION_INSERTDBROWGETIDENTITY_ERROR_1  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
...
function insertDbRow(string $actTab,array $actRow):void
{
 ...
 $result = sqlsrv_query($conn,$query) or die(SQL_SERVER_FUNCTION_INSERTDBROW_ERROR_1  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
 sqlsrv_free_stmt($result); 
 sqlsrv_close($conn);	
}
...
$conn = sqlsrv_connect(SQLSRV_HOST,$sqlsrv_connectionInfo);
 ...
 else
 {
  ...
  {
   if ($i < $num - 1)
    $actOrd = $actOrd . $actOrdField[$i] . STRING_COMMA;
  }
 ...
 die(SQL_SERVER_FUNCTION_GETALLORDDATA_ERROR_1  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
...
$actKeyValue,string $actOrdField,string $actField):array
{
 ...
 $result = sqlsrv_query($conn,$query,array(),array('scrollable'=>SQLSRV_CURSOR_FORWARD)) or 
 die(SQL_SERVER_FUNCTION_GETFILTEREDORDDATA_ERROR_1  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
 $row = array();
...
function getFilteredData(string $actTab,string|array $actKeyField,int|string|array $actKeyValue,string $actField):array
{
 ...
 $result = sqlsrv_query($conn,$query,array(),array('scrollable'=>SQLSRV_CURSOR_FORWARD)) or 
 die(SQL_SERVER_FUNCTION_GETFILTEREDDATA_ERROR_1  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
...
//
// Esegue sql-transact statement
//
function doStmt(string $actTSql,array $actParams=array()):void
{
 $sqlsrv_connectionInfo = getConnectionInfo();
 $conn = sqlsrv_connect(SQLSRV_HOST,$sqlsrv_connectionInfo);	
 ...
 $stmt = sqlsrv_prepare($conn, $actTSql, $actParams);  
 if(! $stmt )  
 {  
  $error = sqlsrv_errors();
  $e = Creator::create("Exception",STRING_BACKSLASH,$error[0]['message']);
...
function getAllDataByQuery(string $actQuery):array|int|string|bool
{
 ...
 die(SQL_SERVER_FUNCTION_GETALLDATABYQUERY_ERROR_1 .
...
function getAllDataByQueryWithParams(string $actQuery,array $params):array|int|string|false
{
 ...
 die(SQL_SERVER_FUNCTION_GETALLDATABYQUERYWITHPARAMS_ERROR_1  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
...
function getFilteredAllData(string $actTab,string|array $actKeyField,int|string|array $actKeyValue):array
{
 ...
 $query = "select * from " . $actTab . " where (" . $whereClause . ")";
 $result = sqlsrv_query($conn,$query,array(),array('scrollable'=>SQLSRV_CURSOR_FORWARD)) or 
 die(SQL_SERVER_FUNCTION_GETFILTEREDALLDATA_ERROR_1  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
...
function getAllData(string $actTab,string $actField):array
{
 ...
 $result = sqlsrv_query($conn,$query,array(),array('scrollable'=>SQLSRV_CURSOR_FORWARD)) or 
 die(SQL_SERVER_FUNCTION_GETALLDATA_ERROR_1  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
 $row=array();
...
function getDistinctAllData(string $actTab,string $actField):array
{
 ...
 " order by " . $actField;
 $result = sqlsrv_query($conn,$query,array(),array('scrollable'=>SQLSRV_CURSOR_FORWARD)) or 
 die(SQL_SERVER_FUNCTION_GETDISTINCTALLDATA_ERROR_1  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
 $row=array();
...
function getUniqueData(string $actTab,string $actKeyField,int $actKeyValue,string $actField):string|int|bool
{
 ...
 $result = sqlsrv_query($conn,$query,array(),array('scrollable'=>SQLSRV_CURSOR_STATIC)) or
 die(SQL_SERVER_FUNCTION_GETUNIQUEDATA_ERROR_1  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
 $num_results = sqlsrv_num_rows($result);
 $row = array();
...
//
function getExtendedUniqueRow(string $actTab,string|array $actKeyField,int|string|array $actKeyValue):array|bool
{
 ...
 $query = "select * from " . $actTab . " where (" . $whereClause . ")";
 $result = sqlsrv_query($conn,$query,array(),array('scrollable'=>SQLSRV_CURSOR_STATIC)) or 
 die(SQL_SERVER_FUNCTION_GETEXTENDEDUNIQUEROW_ERROR_1  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
 $num_results = sqlsrv_num_rows($result);
 $row=array();
...
string $actPkField,array $actForeignKeys,array $actUniqueKeys)
{
 $sqlsrv_connectionInfo=getConnectionInfo();
 ...
 die(SQL_SERVER_FUNCTION_CREATETABLE_ERROR_1  . STRING_ESC_RETURN . print_r(sqlsrv_errors(),true));
 sqlsrv_free_stmt($result);
 sqlsrv_close($conn);
}
//}
