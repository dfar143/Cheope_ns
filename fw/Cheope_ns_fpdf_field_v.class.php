...
class Cheope_ns_fpdf_field_v extends Data_interface
{
 ...
 const POINT_DIM_X = 0.005;
 const POINT_DIM_Y = 0.3528;
 ...
 static private $fpdfFieldVTotNum=0;
  
 function __construct($actObj=OBJ_NONE,string $actOp=OP_NONE,$actNum=STRING_NULL)
 {
  self::$fpdfFieldVTotNum++;
  if($actNum === STRING_NULL)
 	 $actNum = self::$fpdfFieldVTotNum - 1; 
  parent::__construct($actObj,$actOp,self::INT_FPDF_FIELD_V,$actNum);
 }
 ...
 function putFpdf(string $actName,$actValue,string $actDomain):void
 {
	$pdf = $this->getFpdf();
	if(! is_null($pdf))
	{
	...
	$yCoord = $this->getYCoord();
	$height = $this->getHeight();
	$singleCharSizeX = $fontSize * self::POINT_DIM_X;	
	$singleCharSizeY = $fontSize * self::POINT_DIM_Y; 
	$labelName = $this->getLabelName();
	...
	($actDomain !== Int_domain::FIELD_DOMAIN_STRING_PHP_CODE)&&($actDomain !== Int_domain::FIELD_DOMAIN_FUNCTION))
  {
   ...
   $pdf->SetY($yCoord,false);
	 ...
	 $yCoord += $singleCharSizeY;
	 $pdf->SetY($yCoord,false);
	 $num = strlen($label);
	 $xCoord -= ($singleCharSizeX * $num);
	 $pdf->SetX($xCoord);
