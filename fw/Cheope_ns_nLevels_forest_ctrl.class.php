...
class Cheope_ns_nLevels_forest_ctrl extends Cheope_ns_forest_ctrl
{
 const VOICES_POS=0;
 const PAGES_POS=1;
 const IDS_POS=2;
 ...
 const ERROR_1="Cheope_ns_nLevels_forest_ctrl:Oggetto dati non valido.";
 const ERROR_2="Cheope_ns_nLevels_forest_ctrl:Numero campi dati errato.";
...
function putMenuPars(array $actDataValues):void
{
 ...
 $num = count($dataFields);
  ...
  { 
	 $htmlWriter->putGenericHtmlString("var " . $intCode . VAR_SEP . 
	 self::JAVASCRIPT_VAR_LABEL . VAR_SEP . $j . " = " . "\"" . $labelKey . "\"" . ";");		
	 if ($page != NO_VALUE)
	  {
	   if (isset($idsKeys[$j]))
	   {
	  ...
	  else
    {
	 ...
	 $htmlWriter->putGenericHtmlString("var " . $intCode . VAR_SEP . 
	 self::JAVASCRIPT_VAR_PAR . VAR_SEP .  $j . " = " . "\"" .  $opGes . "\";"); 	
