...
abstract class Data_interface extends Generic_interface implements Init_1
{
 Use Creator;
// const ERROR_1="Data_interface:Errore nell'inserimento del grafo del database.";
 const ERROR_2="Data_interface:Errore nell'inserimento del Db_node.";
// const ERROR_3="Data_interface:Errore nell'inserimento dell'int fields container.";	
 const ERROR_4="Data_interface:Errore in setDataFieldsDomainsValues:Oggetto dominio campo dati non esistente.";	
 const ERROR_5="Data_interface:Errore in setDataFieldDomainValueByPos: Oggetto dominio campo dati non esistente.";	
 const ERROR_6="Data_interface:Errore Grafo del database vuoto.";
 const ERROR_7="Data_interface:Errore Contenitore delle queries vuoto.";
// const ERROR_8="Data_interface:Errore nell'inserimento del contenitore queries.";
 const ERROR_9="Data_interface:Errore Dominio non valido.";
 const ERROR_10="Data_interface:Errore Oggetto non valido.";
 const ERROR_11="Data_interface:Errore Dominio non settato per il campo.";
 const ERROR_12="Data_interface:L'oggetto dati non pronto.";
// const ERROR_12="Data_interface:l'Argomento deve essere un array.";
 ...
 // un oggetto Db_node, da un'operazione, da un numero progressivo e
 ...
 static $dataInterfacesTotNum = 0;
 
 function __construct($actObj,string $actOp=STRING_NULL,string $actType,$actNum=STRING_NULL)
 {
  $this->obj = $actObj;
  $this->intFieldsContainer = $this->createIntFieldsContainer();
 ...
 static function isValidDomain(string $actDomain):bool
 {
 	...
 	Int_domain::FIELD_DOMAIN_TABLE,Int_domain::FIELD_DOMAIN_TABLE_NO_LOOKUP,
 	Int_domain::FIELD_DOMAIN_TABLE_UNIQUE_VAL,
 	Int_domain::FIELD_DOMAIN_SET,Int_domain::FIELD_DOMAIN_OBJ,
 	Int_domain::FIELD_DOMAIN_CHECK,Int_domain::FIELD_DOMAIN_RADIO,
 	Int_domain::FIELD_DOMAIN_STATIC_TEXT,Int_domain::FIELD_DOMAIN_MULTIPLE,
 	Int_domain::FIELD_DOMAIN_PASSWORD,
 	Int_domain::FIELD_DOMAIN_FILE,Int_domain::FIELD_DOMAIN_HIDDEN,
 ...
 {
 	$serializer = new Json_serializer(CLIENT_JSON_PATH . DIR_SEP . self::getJsonDataSourceCanonicalName($actObj,$par1));
 	return $serializer; 
 }
 ...
 function convertToKeysNumeric(array $actArray,string $actDefaultValue=STRING_NULL):array
 {
 	$newArray = array();
 	$isPartAssoc = array_is_part_assoc($actArray);
 	$isNumeric = array_is_numeric($actArray) || (count($actArray)==0);
 	$dataFields = $this->getDataFields();
 ...
 array $actFields,string $actDefaultValue=STRING_NULL):array
 {
 	$newArray = array();
 	$isPartAssoc = array_is_part_assoc($actArray);
 	$isNumeric = array_is_numeric($actArray) || (count($actArray)==0);
 ...
 {
// 	if(is_a($actDbStruct,Classes_info::DB_NODES_CONTAINER_CLASS))
...
//   die(self::ERROR_1);
 ...
 {
// 	if(is_a($actDbQueries,Classes_info::QUERIES_CONTAINER_CLASS))
...
/*  elseif($actObj=='OBJ_NONE')
 ...
 {
// 	if(is_a($actIntFieldsContainer,Classes_info::INT_FIELDS_CONTAINER_CLASS))
...
// 	 die(self::ERROR_3);
 ...
 function addField(string $actFieldName,mixed $actFieldDomainValue=Int_domain::FIELD_DOMAIN_VALUE_NONE,
...
// 
 ...
 function getDataFieldDomainByPos(int $actPos):string
 {
...
//   die(self::ERROR_12);    
 ...
 function setDataFieldDomainObjByPos(int $actPos,?Int_domain $actDomObj)
 {
 	$dataFieldsDomainsObjs = $this->getDataFieldsDomainsObjs();
 	if(isset($dataFieldsDomainsObjs[$actPos]))
 	{
 ...
 function getDataFieldType(string $actField)
 {
  $obj = $this->getObj();
 ...
 {
 	if((! isset($this->dataSource)) || (! is_array($this->dataSource)) || (is_array($this->dataSource) && count($this->dataSource)==0))
 	{
 	 $obj = $this->getObj();
     if(is_object($obj) && $obj->ready())
     {
 	  if(is_a($obj,Classes_info::DB_ITEM_CLASS))
 	  {
 ...
 // Ritorna il valore del campo $actFieldName (che può anche non essere uno scalare) in base a
 ...
 array|string|int|float|null $actFieldVal=FIELD_NO_VALUE):mixed
 {
  ...
  $scope = STRING_BACKSLASH . NS .  
   STRING_BACKSLASH ;
  if(is_a($domObj,$scope . Int_domain::CLASS_PREFIX))
