...
class PutBody_array extends PutBody_base
{
 ...
 static function filterInterfacesFiles(string $actIntStr,array $actVet):array
 {
  ...
  $parents = Xml_interface_serializer::getFamilyParents($appName,$actIntStr);
	...
	{		
		...
		$items = $this->getItems();
		$interfaceDivTagContainer1 = $this->getDivTagContainer();
      ...
      $attribs = array("style"=>"float:left;width:180px;");
      ...
      $interfaceDivTagContainer1->add($intDiv1);
      $interfaceDivTagContainer1->add($intHtmlFragment1);
   	//	$intText1 = Creator::create("Html_textarea_tag",STRING_NULL,STRING_NULL);
   		$intText1 = Creator::create(Interfaces_info::INT_HTML_TEXTAREA_TAG,STRING_NULL,STRING_NULL);
   		$attribs2 = array(); 
   		$attribs2["rows"] = "10";
   		$attribs2["cols"] = "35";
   		$attribs2["id"] = $ind;
   		...
   		//$intText1->setTagBody(var_export($item,true));
   		$intText1->setTagBody(var_export($item,true));
   		...
   		$interfaceDivTagContainer1->add($intBr4);
   		if(($ind=="dataFields")||
      ...
      $interfaceHtmlDataTemplate1->setHtmlTemplate("<div dojoType=\"dijit.Menu\" targetNodeIds=\"{TEMP_1}\" style=\"display:none\" >" .
      "{TEMP_2}</div>");  
        ...
        ($items["type"]==Interfaces_info::INT_FORM_SECTION))
         {
   	    ...
   	    {
   	     $fileItems = preg_split("/" . self::INTERFACE_NAME_SEP . "/",$interfaceFile);
   	     if(($fileItems[0] != STANDARD_MOD_PREFIX)&&($interfaceFile != $interfaccia))
   	     {
   			 $fieldDomainValue = $fieldDomainValue . 
   			 self::getDojoMenuItemStr($ind,$interfaceFile);;
   	     }
   	    }
   		 $interfaceHtmlDataTemplate1->setDataFieldDomainValueByPos(1,$fieldDomainValue);
   		}
   	   ...
   	   $interfaceDivTagContainer1->add($interfaceHtmlDataTemplate1);  		
	}
}
...
class PutBody_container extends PutBody_base
{
	...
	function __construct(string $actInd,$item,Interfaces_container $actDivTagContainer)
	{
		parent::__construct($actInd,$item,$actDivTagContainer);
	}
  ...
  $parents = Xml_interface_serializer::getFamilyParents($appName,$actIntStr);
    ...
    $interfaceSelectTag1 = $this->getIntSelectTag();
		$interfaccia = $_GET["Par"]; 
		 ...
		 $intLabel1 = Creator::create(Interfaces_info::INT_HTML_LABEL_TAG,STRING_NULL);
     $attribs3 = array("for" =>$ind,"style"=>"padding-left:230px");
     ...
     $intHtmlFragment2->setHtmlFragment("--->"); 
     $intHtmlFragment2->setDivEnvelope(false);  
     ...
     "dojoType"=>"dojo.dnd.Source","multiple"=>STRING_NULL);
   	 ...
   	 foreach($objNames as $ind2=>$objName)
     {
    	...
    	$attribs2 = array("value"=>$ind2,"class"=>"dojoDndItem");
     ...
     $attribs6["multiple"] = STRING_NULL;
     $attribs6["style"] = "float:left;";
     $interfaceSelectTag1b->setAttribs($attribs6);

     $attribs7 = $interfaceSelectTag1b->getAttribs();
     $attribs7["onchange"] = "select_container_interfaces_onChange($k,'$ind')";
     $interfaceSelectTag1b->setAttribs($attribs7);
     ...
     "style"=>"float:left;cursor:pointer;","title"=>LABEL_IMPOSTA_NODO,"onclick"=>
     "select_container_interfaces_onChange($k,'InterfacesContainer');"));
     ...
     $intDivTagContainer2->add($intHtmlFragment2);
     ...
     $intDivTag2->setDispFields(array(LABEL_CONTENITORE_INTERFACCE));   
     $interfaceDivTagContainer1->add($intDec1);
     $interfaceDivTagContainer1->add($intBr5);    	 
...
//	
   ...
   $className7 = $scope . STRING_BACKSLASH . GENERIC_INTERFACE_CLASS;
     ...
     $attribs4 = array("name" => "select_" . $k,"id" => "select_" . $k ,
     "onchange" => "\$('#$ind').get(0).value = this.value;"  .
     "var len = this.value.length;\$('#$ind').get(0).size=len+10;");
     ...
     $interfacesSelectContainer3 = Creator::create(getClassNameForCreate(Classes_info::INTERFACES_CONTAINER_CLASS),STRING_NULL,STRING_NULL);
   	 ...
   	 foreach($objNames1 as $ind=>$objName)
     {
     	$interfaceOptionTag = Creator::create(Interfaces_info::INT_HTML_OPTION_TAG,STRING_NULL);
    	$attribs5 = array("value"=>$objName);
   	  $interfaceOptionTag->setTagBody(strComplete(ucFirst($ind),ENTITY_SPACE,20));
     ...
     $intDivTag2->setInterfacesContainer($intDivTagContainer2);
     $intDec1 = Creator::create(Interfaces_info::INT_HTML_FIELDSET_DECORATOR,STRING_NULL,$intDivTag2); 
     $intDivTag2->setDispFields(array(LABEL_CONTENITORE_NODI));      		
   	 $interfaceDivTagContainer1->add($intDec1); 
...
class PutBody_at extends PutBody_base
{
  private $intHtmlFragment;
	...
	function exec():void
	{
      ...
      $intDiv1->setAttribs($attribs);
      $intDivContainer = Creator::create(getClassNameForCreate(Classes_info::INTERFACES_CONTAINER_CLASS),STRING_NULL);
      $intDiv1->setInterfacesContainer($intDivContainer);
      $intDiv1->getInterfacesContainer()->add($intLabel1);
      $interfaceDivTagContainer1->add($intDiv1);
      $interfaceDivTagContainer1->add($intHtmlFragment1);
   		...
   		$interfaceDivTagContainer1->add($intBr9);
   		$intBr10 = Creator::create(Interfaces_info::INT_HTML_BR_TAG,STRING_NULL);
   		$interfaceDivTagContainer1->add($intBr10);   		
   ...
   parent::__construct($actInd,$item,$actDivTagContainer);
	...
	function exec():void
	{
      ...
      $interfaceDivTagContainer1->add($intHtmlFragment1);
   		...
   		$attribs2["name"] = $ind;
   		$intInput1->setAttribs($attribs2); 		
   		$interfaceDivTagContainer1->add($intInput1);
   		$intBr9 = Creator::create(Interfaces_info::INT_HTML_BR_TAG,STRING_NULL);
   		$interfaceDivTagContainer1->add($intBr9);
   		$intBr10 = Creator::create(Interfaces_info::INT_HTML_BR_TAG,STRING_NULL);
   		$interfaceDivTagContainer1->add($intBr10);   		
...
class PutBody_object extends PutBody_base
{
  ...
  function setInterfacesFiles(array $actInterfacesFiles):void
  {
	...
	{
		$scope = $this->getScope();
		$className = $scope . STRING_BACKSLASH . INTERFACE_AS_STRING_CLASS;
		$ind = $this->getInd();
		$item = $this->getItem();
      ...
      $interfaceDivTagContainer1->add($intDiv1);
      $interfaceDivTagContainer1->add($intHtmlFragment1);
   		//$intInput1 = Creator::create(Interfaces_info::INT_HTML_INPUT_TAG,STRING_NULL,STRING_NULL);
   		$intInput1 = Creator::create(Interfaces_info::INT_HTML_INPUT_TAG,STRING_NULL);
   		$attribs2 = $intInput1->getAttribs();
   		$attribs2["value"] = $item;
   		$attribs2["id"] = $ind;
   		...
   		$interfaceDivTagContainer1->add($intBr11);
   		$intBr12 = Creator::create(Interfaces_info::INT_HTML_BR_TAG,STRING_NULL);
   		$interfaceDivTagContainer1->add($intBr12);
      ...
      $interfaceDivTagContainer1->add($intHtmlDataTemplate2);	   		
   ...
   $className8 = $scope . STRING_BACKSLASH . INTERFACE_AS_STRING_CLASS;   
      ...
      $interfaceDivTagContainer1->add($intHtmlFragment1);
       ...
       $intLabel2->setAttribs($attribs);
   		 ...
   		 $intInput1->setAttribs($attribs2);
   		 $interfaceDivTagContainer1->add($intInput1);
       $interfaceDivTagContainer1->add($intLabel2);
   		 $interfaceDivTagContainer1->add($intInput2);
   		}
   		elseif($ind=="op")
   		{
...
//
 ...
 echo "}>";*/
  ...
  {
  	$obj = Creator::create(getClassNameForCreate(Classes_info::PUTBODY_OBJECT_CLASS),STRING_NULL,$ind,$item,$divTagContainer);
		$obj->setIntHtmlFragment($intHtmlFragment);
		$obj->setInterfacesFiles($interfacesFiles);
		$obj->setScope($scope);
  }
