...
class Std_pop3Mail extends Generic_interface
{
...
// $actConnection è un IMAP stream.
//
function pop3_retr(IMAP\Connection $actConnection,int $actMessageNum):string|bool
{
    return(imap_fetchheader($actConnection,$actMessageNum,FT_PREFETCHTEXT));
}

//
// $actConnection è un IMAP stream.
//
function pop3_delete(IMAP\Connection $actConnection,string $actMessage):bool
{
...
// $actConnection è un IMAP stream.
//
function mail_decode_part(IMAP\Connection $actConnection,int $actMessageNumber,stdClass|false $actPartObj,string $actPrefix)
{
    $attachment = array();
