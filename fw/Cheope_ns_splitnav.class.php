...
class Cheope_ns_splitnav extends Html_data_interface
{ 
 const ERROR_1="Cheope_ns_splitnav.putData:Numero campi dati errato.";
 const DEFAULT_CSS_CLASS="split_topnav";
 const DEFAULT_CSS_MODULE="splitnav";
 const TITLES_POS=0;
 const PAGES_POS=1;
 const LINKS_POS=2;

 private $gesPage=STRING_NULL;
 private $anchor=STRING_NULL;
 ...
 function __construct($actObj=OBJ_NONE,string $actOp=OP_NONE,$actNum=STRING_NULL)
 {
  ...
  parent::__construct($actObj,$actOp,self::INT_SPLITNAV,$actNum);
 ...
 function setAnchor(string $actAnchor):void
 {
  $this->anchor = $actAnchor;
 }
 ...
 $pagesField = $this->getPagesField();
  ...
  foreach($titles as $ind=>$title)
  {
    ...
    $htmlWriter->putHrefOpenTag(STRING_NULL,STRING_NULL,($i==0)?"active":STRING_NULL,$link);
	$htmlWriter->putGenericHtmlString($title);
    $htmlWriter->putGenericHtmlString(ANCHOR_CLOSE_TAG);	
    $i++;
  }
  $anchor = $this->getAnchor();
  $htmlWriter->putHrefOpenTag(STRING_NULL,STRING_NULL,"split",$link);
  $htmlWriter->putGenericHtmlString($anchor);
  $htmlWriter->putGenericHtmlString(ANCHOR_CLOSE_TAG);  
