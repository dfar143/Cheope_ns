...
require_once(FRAMEWORK_PATH . "Cheope_ns_prova_46_op_page.class.php");

//
// Permette il cross site loading 
//
 $intHtmlButton1 = new Html_button_tag(OP_NONE);
 $attribs1 = array("type"=>"button","onclick" =>
 "dojo.io.script.get({" . 
 "callbackParamName:'cucu'," .
 "url:'ajax_handler.php?Op6=ciao'," .
 //"jsonp_dojoIoScript1:{'_jsonpCallback':'WWWWW'},".
 ...
 "}," .
 "load:function(data){" .
 "console.log('success xhrGet',data);" .
 "dojo.byId('html_tags__2').innerHTML='<pre>' + dojo.toJson(data, true) + '</pre>'" .
 "}," .
 "error:function(response,ioArgs){" .
 "console.log('failed xhrGet',response,ioArgs);" .
 "return response;" .
...
/* $interfacesContainer->add($interfaceFrame1);
 ...
 $page = new Cheope_ns_prova_46_op_page();
 $page->setInterfacesContainer($interfacesContainer);
 $page->setJQueryEnabled(true);
 $page->setDojoEnabled(true);
 $page->setCREnabled(true);
 $page->setAjaxOps(array(AJAX_OP_OP6));
 $page->putData();
