...
//
// Aggiunge un nuovo gruppo per la selezione di chiavi candidate
//
...
//
// Aggiunge il gruppo per la selezione delle chiavi esterne
//
function insertHtmlTags2NewGroup()
{
  ...
  $('#html_tags__2_lista_nuove_tabelle_0').append('<option value=""></option>');
  ...
  $('#external_key').bind('change',function(){
    ...
    form_inserimento_4_input_ext_key_onChange(this);
  	});
...
function form_inserimento_1_lista_tabelle_onChange()
{
 ...
 var ids=(tabId=='')?'false':tabId;
 ajaxHandler.synServerCall('ajax_handler.php','getAllFieldsDefProps',ids,'xml',/ind_records/);
 //console.log(ids);
 if(ids != 'false')
  ajaxHandler.serverCall('ajax_handler.php','getPkKeyField',ids,'text',/[\s\._\:A-Za-z0-9;\-\?\-_\"\']*/);
}
...
{	
 ...
 {
 	$('#form_2__2_submit_button_id').get(0).onclick();
 	$('#form_2__4_submit_button_id').get(0).onclick();
  $('#form_2__3_submit_button_id').get(0).onclick();
...
function form_inserimento_2_lista_campi_onChange(actObj)
{
 ...
 var field = $('select#Lista_campi option:selected').text();
 ...
 function form_inserimento_3_reset_button_onClick(actName)
 {
  if($('select#Lista_tabelle option:selected').text()=='')
  {
   alert(loc.getString('msg',11));
 	...
 	var tabId = $('#Lista_tabelle option:selected').val();
  var ids = tabId;
  ajaxHandler.serverCall('ajax_handler.php','getCandKeyFieldsProps',ids,'xml',/[<>"\s\._\:A-Za-z0-9;\-\?\\=]*/);
  alert(loc.getString('msg',91));
  return false;
 }
...
// alert('setFieldsDefExtKeyFieldsProps:' + ids);
 ajaxHandler.synServerCall('ajax_handler.php','setFieldsDefWithoutExtKeys',tab,'text',/[.]*\w[.]*/);
 ajaxHandler.synServerCall('ajax_handler.php','setFieldsDefExtKeyFieldsProps',ids,'text',/[.]*\w[.]*/);
 ajaxHandler.synServerCall('ajax_handler.php','setFieldsDef',tab,'text',/[.]*\w[.]*/);
 ajaxHandler.synServerCall('ajax_handler.php','setFieldsConstsDef',ids2,'text',/[.]*\w[.]*/);
 ...
 function form_inserimento_4_reset_button_onClick(actName)
 {
  if($('select#Lista_tabelle option:selected').text()=='')
  {
   alert(loc.getString('msg',11));
 	...
 	var tabId = $('#Lista_tabelle option:selected').val();
  var ids = tabId;
  ajaxHandler.serverCall('ajax_handler.php','getExtKeyFieldsProps',ids,'xml',/[.]*\w[.]*/);
  alert(loc.getString('msg',91));
  return false;
 }
...
{
 if(($('select#Lista_tabelle option').length >= 1)&&
 ($('select#Lista_tabelle option:first').text()!=''))
 {
  display_exec_confirm_dialog_2();	
 }
 else
 {
...
function button_2_onClick(actObj)
{
	if($('select#Lista_tabelle option:selected').text()!='')
  {
	 ...
	 subModal.showPopWin('view_all_fields.php?Par=' + tablePos,600,400,
	 function(actVar){$('#form_2__1_reset_button_id').get(0).onclick();},true);
  }
}
    ...
    $( "#dialog-confirm" ).dialog({
      ...
      buttons: {
        "Create table": function() {
          $( this ).dialog( "close" );
          var tableName = $('select#Lista_tabelle option:selected').text().replace(/\s*/g,'');
          ajaxHandler.synServerCall('ajax_handler.php','sqlServerCreateTable',tableName,'text',/[\s\._\:A-Za-z0-9;\-]*/);
        },
        Cancel: function() {
          $( this ).dialog( "close" );
        }
...
function display_exec_confirm_dialog_2()
{
  $(function() {
    $( "#dialog-confirm-2" ).dialog({
      ...
      modal: true,
      buttons: {
        "Create db structure file": function() {
...
function form_inserimento_4_input_ext_key_onChange(actObj)
{
  var tableName =$("#html_tags__2_lista_tabelle_0").val();
  //alert('#' + tableName + '#');
  var newVal = $(actObj).val();
  ajaxHandler.synServerCall('ajax_handler.php','getPkKeyByTableName',tableName,'text',/[\s\._\:A-Za-z0-9;\-]*/);
  ...
  if((res !== newVal)&&(newVal !== ''))
  {
	...
	$(actObj).val(res);
    var selectCtrl = $('#html_tags__2_lista_tabelle_0 option:selected');
    selectCtrl.each(function()
    {
 	 var field = $(this).data('keyField',res);
	});
	//$(actObj).data('keyField',res);
	return false;
  }
