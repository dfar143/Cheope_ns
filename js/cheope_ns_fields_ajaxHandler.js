 ...
 function OpGetExtKeyFieldsProps(actName)
 {
  ...
  $('#html_tags__2_group_0').remove();
 	insertHtmlTags2NewGroup();
 	var extKeysTables = childs[0].childNodes;
 	var extKeysFields  = childs[1].childNodes;
 	var num3 = extKeysTables.length;
 	//alert(num3);
 	var firstExtKey = '';
 	for(var l=0;l<=num3-1;l++)
 	{
 	 if(extKeysTables[l].childNodes[0].nodeValue!=null)
 	 {
 	  var extTable = extKeysTables[l].childNodes[0].nodeValue;
 	  var extKey = extKeysFields[l].childNodes[0].nodeValue;
 	  $('#html_tags__2_lista_tabelle_0').append('<option>' + extTable + '</option>');
 	  $('#html_tags__2_lista_tabelle_0 option:last').data('keyField',extKey);
 	  if(l==0)
 	   firstExtKey = extKey;
 	 }
 	}
 	$('#html_tags__2_lista_tabelle_0').append('<option></option>');
 	$('#external_key').val(firstExtKey);
 };
 }
 ...
 function OpCheckIfIsSuitablePkKey(actName)
 {
 	this.name = actName;
 	this.exec = function(actXmlMsg){
 	if(actXmlMsg=='false')alert(loc.getString('msg',37));
 }; 	
 }
 ...
 function OpSqlServerCreateTable(actName)
 {
 	this.name = actName;
 	this.exec = function(actXmlMsg){
